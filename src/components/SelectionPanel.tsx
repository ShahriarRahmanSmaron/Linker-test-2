    ...
    `}>
      ...
      <div className="p-3 overflow-y-auto max-h-[40vh] md:max-h-[300px] space-y-2 bg-neutral-50/50">
        {selectedFabrics.map(fabric => (
            <div key={fabric.id} className="flex items-center bg-white p-2 rounded-xl border border-neutral-100 shadow-sm animate-fade-in group hover:border-primary-200 hover:shadow-md hover:translate-x-1 transition-all duration-200">
                <div className="h-10 w-10 rounded-lg bg-neutral-200 flex-shrink-0 relative overflow-hidden mr-3 shadow-inner">
                    <div className="absolute inset-0 mix-blend-multiply opacity-80" style={{ backgroundColor: fabric.color }}></div>
                </div>
                <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-bold text-neutral-800 truncate group-hover:text-primary-700 transition-colors">{fabric.name}</h4>
                    <p className="text-[10px] text-neutral-500">{fabric.fabrication}</p>
                </div>
                <button 
                    onClick={(e) => { e.stopPropagation(); onRemove(fabric.id); }}
