        ...
        <Reveal>
          <div className="text-center mb-8 sm:mb-12">
            ...
            <div className="flex justify-center mt-6 sm:mt-8">
              <div className="bg-white dark:bg-neutral-800 p-1 rounded-full border border-neutral-200 dark:border-neutral-700 shadow-sm inline-flex relative">
                <div 
                  className={`absolute top-1 bottom-1 rounded-full transition-all duration-300 ease-out shadow-sm ${activeTab === 'buyer' ? 'left-1 w-[120px] sm:w-[140px] bg-primary-500' : 'left-[125px] sm:left-[145px] w-[150px] sm:w-[180px] bg-accent-500'}`}
                ></div>
                <button 
                  onClick={() => setActiveTab('buyer')}
                  className={`relative px-4 sm:px-8 py-2 rounded-full text-xs sm:text-sm font-bold transition-colors z-10 w-[120px] sm:w-[140px] ${activeTab === 'buyer' ? 'text-white' : 'text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-200'}`}
                >
                  I'm a Buyer
                </button>
                <button 
                  onClick={() => setActiveTab('manufacturer')}
                  className={`relative px-4 sm:px-8 py-2 rounded-full text-xs sm:text-sm font-bold transition-colors z-10 w-[150px] sm:w-[180px] ${activeTab === 'manufacturer' ? 'text-white' : 'text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-200'}`}
                >
                  I'm a Manufacturer
                </button>
              </div>
        ...
        <Reveal delay={500}>
          <div className="mt-12 sm:mt-20 text-center">
             <button 
                ...
                className={`px-6 sm:px-8 py-2.5 sm:py-3 text-white rounded-full text-sm sm:text-base font-bold shadow-lg transition-all duration-200 transform hover:scale-[1.02] flex items-center mx-auto ${buttonClass}`}
              >
